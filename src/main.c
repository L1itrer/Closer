...
int main(int argc, char* argv[], char* env[])
{
  ...
  //close(g_connfd);
  // NOTE: there is a very weird bug on xwayland where sometimes
  // atoms will not be interned correctly, it is unformtunately
  // not easily reproducible, might have to investigate
  bool32 res = x11_init_connection(&state);
  if (res != TRUE) return 1;
